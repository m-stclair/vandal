...
void main() {
    ...
    for (int i = -u_kernelRadius; i <= u_kernelRadius; i++) {
        for (int j = -u_kernelRadius; j <= u_kernelRadius; j++) {
            float w = u_kernel[i + u_kernelRadius] * u_kernel[j + u_kernelRadius];
            vec2 uvk = uv + vec2(float(i), float(j)) * texelSize;

            // Sobel at uvk
            vec3 tl = texture(u_image, uvk + vec2(-1,-1)*texelSize).rgb;
            vec3 tc = texture(u_image, uvk + vec2( 0,-1) * texelSize).rgb;
            vec3 tr = texture(u_image, uvk + vec2( 1,-1) * texelSize).rgb;
            vec3 ml = texture(u_image, uvk + vec2(-1, 0) * texelSize).rgb;
            vec3 mr = texture(u_image, uvk + vec2( 1, 0) * texelSize).rgb;
            vec3 bl = texture(u_image, uvk + vec2(-1, 1) * texelSize).rgb;
            vec3 bc = texture(u_image, uvk + vec2( 0, 1) * texelSize).rgb;
            vec3 br = texture(u_image, uvk + vec2( 1, 1) * texelSize).rgb;
